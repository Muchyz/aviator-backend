...
function crashPointFromSeed(seed) {
  ...
  return parseFloat(Math.max(1.0, raw / 100).toFixed(2));
}
...
app.post("/api/admin/users/:id/balance", adminAuth, async (req, res) => {
  ...
  try {
    ...
    await pool.query("INSERT INTO transactions (user_id,type,label,amount) VALUES($1,$2,$3,$4)", [req.params.id, amount>0?"dep":"wd", note||"Admin adjustment", parseFloat(amount)]);
    res.json({ ok: true, balance: parseFloat(updated.rows[0].balance) });
  } catch { res.status(500).json({ error: "Failed to adjust balance" }); }
});
